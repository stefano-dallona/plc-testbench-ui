...
RUN git clone --branch 13-apply-changes-needed-to-integrate-with-web-gui https://github.com/LucaVignati/plc-testbench.git && cd plc-testbench && python setup.py sdist && python3 -m pip install -f ./dist plc-testbench && cp -r dl_models /plc-testbench-ui
